...
export default function ChartOfAccountsPage() {
    ...
    const handleToggleActive = async (account: FinancialClass) => {
        ...
        try {
            ...
            if (error) throw error;
            await fetchAccounts();
            toast.success(`Conta ${account.active ? 'desativada' : 'ativada'} com sucesso!`);
        } catch (error: unknown) {
            await alert('Erro', getErrorMessage(error), 'danger');
        } finally {
            setLoading(false);
    ...
    return (
        <div style={{ padding: '40px', minHeight: '100vh', background: '#f8fafc' }}>
            <PageHeader
                ...
                description="Estrutura contábil GAAP-compliant - Wind Wireless LLC"
                icon="📋"
                breadcrumbs={[
                    { label: 'FINANCEIRO', href: '/finance', color: '#059669' },
                    { label: 'PLANO DE CONTAS CONTÁBIL', color: '#059669' },
                ]}
                moduleColor="#059669"
                actions={
                    <button
