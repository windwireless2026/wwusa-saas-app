    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editing) {
                ...
                if (error) throw error;
                await alert('Sucesso', 'Grupo criado!', 'success');
            }
            await fetchGroups();
            closeModal();
        } catch (error: any) {
    ...
    return (
        <div style={{ padding: '40px', minHeight: '100vh', background: '#f8fafc' }}>
            ...
            <div style={{ marginBottom: '24px', fontSize: '14px', color: '#64748b' }}>
                📋 <a href="/dashboard/registration" style={{ fontWeight: '600', color: '#3b82f6', textDecoration: 'none', cursor: 'pointer' }}>{t('breadcrumb')}</a> › {t('groups.title')}
            </div>
